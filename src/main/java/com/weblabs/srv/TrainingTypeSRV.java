	      ...
	      List<AddTraining_Type> expenses;
	        ...
	        if (typeFilter != null && !typeFilter.isEmpty() || idFilter != null && !idFilter.isEmpty()) {
	        	...
	        	if (idFilter == null || idFilter.isEmpty()) {
	        	 int idFilterInt = 0;
	        	 expenses = AddTraining_TypeDAO.getFilteredAddTrainingtype("type like '%" + typeFilter + "%' or id = '" + idFilterInt + "'", start, limit);
	        	}
	        	else {
	        		expenses = AddTraining_TypeDAO.getFilteredAddTrainingtype("type like '%" + typeFilter + "%' or id = '" + idFilter + "'", start, limit);
	        	}
	        	} else {
	            // Retrieve all data
