...
<%
    ...
    if (!whereClause.isEmpty()) {
        ...
        overtime = OvertimeDAO.getFilteredOvertimes(whereClause, start, limit);
    }

    if (overtime != null && !overtime.isEmpty()) {
    	AddOverTime time =overtime.get(0); // Access the first element
        if (overtime != null) {
%>
...
<div class="row">
                    <div class="col-sm-6">
                        ...
                        <div class="form-group">
                            ...
                            <input name="employee" value="<%= time.getEmployee_Id() %>" readonly required class="form-control" type="text">
                        </div>
              
                        <div class="form-group">
                            <label class="col-form-label">Overtime Date <span class="text-danger">*</span></label>
                            <input name="overtimedate" value="<%= time.getOverTime_Date() %>" required class="form-control" type="text">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input name="type" value="<%= time.getType() %>" required class="form-control" type="text">
                        </div>
                        ...
                        <div class="submit-section">
                            <button type="submit" name="update_overtime" class="btn btn-primary submit-btn">Update</button>
                        </div>
                    </div>
                </div>
                        </form> 
    ...
    if (sessionstatus.getAttribute("status") != null && sessionstatus.getAttribute("status").equals("overtime Employee Position Updated Successfully!")) {
        response.sendRedirect("overtime.jsp");
    } else {
%>
   <div class="col-sm-6">
		<p>overtime emp not found with the provided ID.</p>
</div>
<%
    }
%>
